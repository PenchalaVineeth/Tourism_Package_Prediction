...
# Customer details
Age = st.number_input("Age (Customer's age in years)", min=18, max=100, value=45)
...
NumberOfPersonVisiting = st.number_input('Total number of people accompanying the customer on the trip', min=0, max=10, value=2)
...
NumberOfTrips = st.number_input('Average number of trips the customer takes annually', min=0, max=50, value=2)
...
NumberOfChildrenVisiting = st.number_input('Number of children below age 5 accompanying the customer', min=0, max=3, value=2)
Designation = st.selectbox("Customer's designation in their current organization", ['Executive', 'Manager', 'Senior Manager', 'AVP', 'VP'])
MonthlyIncome = st.number_input('Gross monthly income of the customer', min=1000, max=100000, value=50000)
...
NumberOfFollowups = st.slider('Total number of follow-ups by the salesperson after the sales pitch', min=1, max=10, value=3)
DurationOfPitch = st.number_input('Duration of the sales pitch delivered to the customer', min=3, max=200, value=20)
...
if st.button('Predict'):
  ...
  st.write(f"Based on the information provided customer is '{result}' to purchase the package")
